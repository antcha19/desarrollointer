...
public class GestionEwallet {
    ...
    public static void altawallet(String nombre, String apellidos, String DNI, Date fecha_nacimiento, String email, int saldo_puntos, int saldo_euros) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("comweb");
    ...
    public void borarwallet(int idwallet) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("comweb");
        EntityManager em = emf.createEntityManager();
        EntityTransaction tx = em.getTransaction();
    ...
    public List<Ewallet> recuperarwallets() {
        ...
        List<Ewallet> lista_wallets = (List<Ewallet>) qr.getResultList();
        //Terminamos la transaccion
        tx.commit();
        for (int i = 0; i < lista_wallets.size(); i++) {
            Ewallet aux = lista_wallets.get(i);
